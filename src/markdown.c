...
static char **split_lines(const char *text, int *count_out)
{
    ...
    if (!lines) { *count_out = 0; return NULL; }
...
static void render_heading(PDF *pdf, const char *text, int level)
{
    ...
    float h_needed = HEAD_BEFORE + leading + HEAD_AFTER
                   + (level <= 2 ? 4.0f : 0.0f); /* extra for underline */
...
static void render_paragraph(PDF *pdf, const char *text,
                              ...
                              float font_size, int base_font)
{
    ...
    pdf_paragraph(pdf, text, left_indent, right_indent,
                  font_size, base_font, 0.0f);
...
static void render_ulist(PDF *pdf, ListItem *items, int n,
                          float extra_indent)
{
    float ml       = pdf_margin_left(pdf);
    ...
    for (int i = 0; i < n; i++) {
        ...
        pdf_text_line(pdf, "\xe2\x80\xa2",
                      FONT_NORMAL, BODY_SIZE,
                      bullet_x - ml, leading);
        /* Go back up by the same amount */
        pdf_advance_y(pdf, -leading);

        /* Item text — same leading, same starting y */
        float used = pdf_paragraph(pdf, items[i].text,
                                   text_x - ml, 0.0f,
                                   BODY_SIZE, FONT_NORMAL, leading);
        /* If text was shorter than one line height, pad */
        if (used < leading)
            pdf_advance_y(pdf, leading - used);
        pdf_advance_y(pdf, LIST_ITEM_SP);
    }
}

static void render_olist(PDF *pdf, ListItem *items, int n,
                          int start, float extra_indent)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        /* Number */
        pdf_text_line(pdf, num_str, FONT_NORMAL, BODY_SIZE,
                      num_x - ml, leading);
        pdf_advance_y(pdf, -leading);
...
static void render_blockquote(PDF *pdf, const char *text)
{
    ...
    pdf_vbar(pdf, bar_x, y_start, y_end - y_start,
             0.5f, 0.5f, 0.5f, QUOTE_BAR_W);
}
...
int markdown_to_pdf(const char *content, PDF *pdf, const char *input_path)
{
    ...
    char  *para_buf  = NULL;  size_t para_cap  = 0;
    char  *code_buf  = NULL;  size_t code_cap  = 0;
    char  *quote_buf = NULL;  size_t quote_cap = 0;
    char   fence_char = '`';
    ...
    for (int li = 0; li < line_count; li++) {
        ...
        if (li + 1 < line_count) {
            ...
            if (sul && line[0] != '\0' && !atx_level(line)) {
                ...
                switch (state) {
                    case ST_PARA:     FLUSH_PARA();  break;
                    case ST_ULIST:    FLUSH_ULIST(); break;
                    case ST_OLIST:    FLUSH_OLIST(); break;
                    case ST_BLOCKQUOTE: FLUSH_QUOTE(); break;
                    case ST_CODE_INDENT: FLUSH_CODE(); break;
                    default: break;
                }
        ...
        /* ---- indented code block -------------------------------------- */
        if (indented_code(line) && state != ST_PARA &&
            state != ST_ULIST && state != ST_OLIST) {
        ...
        if (line[0] == '>') {
            ...
            if (quote_buf && quote_buf[0])
                ...
                /* first line */
                if (!quote_buf) {
                    quote_buf = malloc(512); quote_cap = 512;
