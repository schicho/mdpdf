...
/* ── font names ─────────────────────────────────────────────────────────── */
static const char* FONT_NAMES[FONT_COUNT] = {"Helvetica",         "Helvetica-Bold",
...
static float PDF_Y(const PDF* pdf, float y_from_top) { return pdf->ph - pdf->mt - y_from_top; }
...
void pdf_free(PDF* pdf) {
    if (!pdf) return;
    for (int i = 0; i < pdf->page_count; i++) free(pdf->pages[i].data);
    free(pdf->pages);
...
void pdf_set_input_dir(PDF* pdf, const char* dir) {
    if (dir) snprintf(pdf->input_dir, sizeof(pdf->input_dir), "%s", dir);
}
...
void pdf_ensure_space(PDF* pdf, float height) {
    ...
    if (available < height) pdf_new_page(pdf);
}

void pdf_advance_y(PDF* pdf, float amount) { pdf->y += amount; }
...
void pdf_rect_fill(PDF* pdf, float x, float y_from_top, float w, float h, float r, float g,
    ...
    buf_printf(&pdf->content, "q %.3f %.3f %.3f rg %.3f %.3f %.3f %.3f re f Q\n", (double)r,
               (double)g, (double)b, (double)x, (double)py, (double)w, (double)h);
}

void pdf_hline(PDF* pdf, float x, float y_from_top, float width, float r, float g, float b,
               float lw) {
    float py = PDF_Y(pdf, y_from_top);
    buf_printf(&pdf->content, "q %.3f %.3f %.3f RG %.2f w %.3f %.3f m %.3f %.3f l S Q\n", (double)r,
               (double)g, (double)b, (double)lw, (double)x, (double)py, (double)(x + width),
               (double)py);
}
...
void pdf_vbar(PDF* pdf, float x, float y_from_top, float height, float r, float g, float b,
    ...
    buf_printf(&pdf->content, "q %.3f %.3f %.3f RG %.2f w %.3f %.3f m %.3f %.3f l S Q\n", (double)r,
               (double)g, (double)b, (double)lw, (double)x, (double)py_top, (double)x,
               (double)py_bot);
}
 ...
 */
static void pdf_add_link_annot(PDF* pdf, float x1, float y1, float x2, float y2, const char* url) {
    if (!url || !*url) return;
    if (pdf->link_annot_count >= pdf->link_annot_alloc) {
        int        na = pdf->link_annot_alloc ? pdf->link_annot_alloc * 2 : 64;
        LinkAnnot* nb = realloc(pdf->link_annots, (size_t)na * sizeof(LinkAnnot));
        if (!nb) return;
        pdf->link_annots = nb;
...
static unsigned char utf8_to_winansi(const char** pp) {
    ...
    *pp = (const char*)(p + 1);
    ...
    switch (cp) {
        ...
        case 0x201A:
            return 0x82; /* ‚ single low-9 quotation */
        case 0x0192:
            return 0x83; /* ƒ */
        case 0x201E:
            return 0x84; /* „ double low-9 quotation */
        case 0x2026:
        ...
        case 0x02C6:
            return 0x88; /* ˆ circumflex */
        case 0x2030:
            return 0x89; /* ‰ per mille */
        case 0x0160:
            return 0x8A; /* Š */
        case 0x2039:
            return 0x8B; /* ‹ */
        ...
        case 0x017D:
            return 0x8E; /* Ž */
        case 0x2018:
            return 0x91; /* ' left single quotation */
        case 0x2019:
            return 0x92; /* ' right single quotation */
        case 0x201C:
            return 0x93; /* " left double quotation */
        case 0x201D:
            return 0x94; /* " right double quotation */
        case 0x2022:
            return 0x95; /* • bullet */
        case 0x2013:
            return 0x96; /* – en dash */
...
float pdf_text_line(PDF* pdf, const char* text, int font, float size, float x_offset,
    ...
    buf_printf(&pdf->content, "BT /F%d %.2f Tf %.3f %.3f Td ", font, (double)size, (double)x,
               (double)py);
 ...
 */
static int parse_inline(const char* src, int base_font, Span* spans, int max_spans) {
    int         n = 0;
    int         cur = base_font;
    const char* p = src;
    char        tmp[512];
...
#define FLUSH_SPAN()                                                   \
    ...
    while (*p) {
        ...
        if ((p[0] == '*' && p[1] == '*') || (p[0] == '_' && p[1] == '_')) {
            ...
            if (end) {
                if (n < max_spans) {
                    spans[n].font = (base_font == FONT_ITALIC) ? FONT_BOLDITALIC : FONT_BOLD;
                    spans[n].is_link = 0;
                    spans[n].url[0] = '\0';
                    size_t len = (size_t)(end - p);
                    if (len >= sizeof(spans[n].text)) len = sizeof(spans[n].text) - 1;
        ...
        if (p[0] == '*' || (p[0] == '_' && (p[1] != ' ' && p[1] != '\0'))) {
            ...
            if (end && (delim == '*' || (end > p + 1 && end[-1] != ' '))) {
                ...
                p++;
                if (n < max_spans) {
                    spans[n].font = (base_font == FONT_BOLD) ? FONT_BOLDITALIC : FONT_ITALIC;
                    spans[n].is_link = 0;
                    spans[n].url[0] = '\0';
                    size_t len = (size_t)(end - p);
                    if (len >= sizeof(spans[n].text)) len = sizeof(spans[n].text) - 1;
            ...
            if (end_bracket && end_bracket[1] == '(') {
                ...
                if (end_paren) {
                    ...
                    if (text_len > 0 && n < max_spans) {
                        ...
                        spans[n].is_link = 1;
                        if (url_len >= sizeof(spans[n].url)) url_len = sizeof(spans[n].url) - 1;
                        memcpy(spans[n].url, end_bracket + 2, url_len);
                        spans[n].url[url_len] = '\0';
                        if (text_len >= sizeof(spans[n].text)) text_len = sizeof(spans[n].text) - 1;
        ...
        /* Regular character */
        if (ti < (int)sizeof(tmp) - 2) tmp[ti++] = *p;
        p++;
    }
    FLUSH_SPAN();
#undef FLUSH_SPAN
 ...
 * Returns token count.
 */
static int build_tokens(const Span* spans, int ns, Token* toks, int max_toks, float font_size) {
    int n = 0;
    for (int s = 0; s < ns && n < max_toks; s++) {
        ...
        while (*p && n < max_toks) {
            ...
            if (*p == ' ') {
                if (n > 0) {
                    ...
                    if (tl + 1 < sizeof(toks[n - 1].text)) {
                        ...
                        toks[n - 1].w =
                            pdf_text_width(toks[n - 1].text, toks[n - 1].font, font_size);
                    }
                }
                p++;
                continue;
            ...
            if (wlen >= sizeof(toks[n].text)) wlen = sizeof(toks[n].text) - 1;
...
float pdf_paragraph(PDF* pdf, const char* text, float left_indent, float right_indent,
    ...
    while (ti < nt) {
        ...
        while (ti < nt) {
            float tw = toks[ti].w;
            if (line_end > line_start && line_w + tw > max_w) break; /* wrap */
            line_w += tw;
            line_end = ti + 1;
            ti++;
        }
        ...
        while (k < line_end) {
            ...
            while (run_end < line_end && toks[run_end].font == run_font &&
                   ...
                   (!run_is_link || strcmp(toks[run_end].url, toks[k].url) == 0))
                run_end++;

            /* Set blue fill colour for hyperlinks */
            if (run_is_link) buf_append(&pdf->content, "0 0 1 rg\n", 9);
            ...
            for (int j = k; j < run_end; j++) {
                ...
                if (run_ti + (int)src_len >= (int)sizeof(run_text) - 1) {
                    ...
                    run_text[run_ti] = '\0';
                    if (run_ti > 0) {
                        buf_printf(&pdf->content, "BT /F%d %.2f Tf %.3f %.3f Td ", run_font,
                                   (double)font_size, (double)run_x, (double)py);
            ...
            if (run_end == line_end) {
                while (run_ti > 0 && run_text[run_ti - 1] == ' ') run_ti--;
                run_text[run_ti] = '\0';
            }
            ...
            if (run_is_link) {
                ...
                if (run_ti > 0) {
                    float ann_x2 = ann_x1 + pdf_text_width(run_text, run_font, font_size);
                    float ann_y1 = py - font_size * 0.2f; /* below baseline */
                    float ann_y2 = py + font_size * 0.8f; /* above baseline */
                    pdf_add_link_annot(pdf, ann_x1, ann_y1, ann_x2, ann_y2, toks[k].url);
                }
            }
...
float pdf_code_block(PDF* pdf, const char* code, float left_indent) {
    ...
    while (line) {
        ...
        pdf_rect_fill(pdf, pdf->ml + left_indent, pdf->y, block_w, leading + CODE_PAD_V, CODE_BG_R,
                      CODE_BG_G, CODE_BG_B);

        /* text */
        float py = PDF_Y(pdf, pdf->y + fs + CODE_PAD_V / 2.0f);
        buf_printf(&pdf->content, "BT /F%d %.2f Tf %.3f %.3f Td ", FONT_MONO, (double)fs,
                   (double)x_left, (double)py);
...
float pdf_image(PDF* pdf, const char* path, float max_width, float max_height) {
    ...
    if (path[0] == '/' || !pdf->input_dir[0]) {
        ...
        if (n > 0 && n < (int)sizeof(full_path) - 2) {
            full_path[n] = '/';
            snprintf(full_path + n + 1, sizeof(full_path) - (size_t)n - 1, "%s", path);
        }
    }
    ...
    if (idx < 0) {
        ...
        snprintf(pdf->images[idx].resname, sizeof(pdf->images[idx].resname), "Im%d", idx);
        snprintf(pdf->images[idx].path, sizeof(pdf->images[idx].path), "%s", full_path);
    }
    ...
    buf_printf(&pdf->content, "q %.3f 0 0 %.3f %.3f %.3f cm /Img%d Do Q\n", (double)w_pts,
               (double)h_pts, (double)x, (double)py, idx);
...
int pdf_write(PDF* pdf, const char* path) {
    ...
    for (int ai = 0; ai < n_annots; ai++) {
        ...
        fbuf_printf(&out,
                    ...
                    oid, (double)la->x1, (double)la->y1, (double)la->x2, (double)la->y2);
    ...
    char font_res[512];
    {
        ...
        for (int fi = 0; fi < FONT_COUNT && rem > 0; fi++) {
            n2 = snprintf(font_res + k, (size_t)rem, "/F%d %d 0 R ", fi, obj_font_base + fi);
            if (n2 > 0 && n2 < rem) {
                k += n2;
                rem -= n2;
            }
    ...
    for (int pi = 0; pi < n_pages; pi++) {
        ...
        fbuf_printf(&out, "%d 0 obj\n<< /Length %zu >>\nstream\n", cs_oid, cs_len);
        if (cs_len > 0) fbuf_write(&out, pdf->pages[pi].data, cs_len);
        ...
        fbuf_printf(&out,
                    ...
                    "   /MediaBox [0 0 %.3f %.3f]\n"
                    "   /Resources << /Font %s",
                    page_oid, (double)pdf->pw, (double)pdf->ph, font_res);
        if (n_images > 0) fbuf_printf(&out, " /XObject %s", xobj_res);
        fbuf_printf(&out, " >>\n");
    ...
    for (int pi = 0; pi < n_pages; pi++) fbuf_printf(&out, "%d 0 R ", obj_page_base + pi * 2);
    fbuf_printf(&out, "] >>\nendobj\n");
